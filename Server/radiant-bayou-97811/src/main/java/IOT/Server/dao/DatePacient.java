...
public class DatePacient implements DAO {
    ...
    public void insertIntoDB2(String CNP,String time, String ... args) throws InvalidNrOfArgsException, InterruptedException, ExecutionException {
        ...
        dataToInsert.put("nivel_oxigen", args[3]);
        dataToInsert.put("calitate_somn", args[4]);

        ApiFuture<WriteResult> insertData = database.db.collection("pacient").document(CNP).collection("date").document(time).set(dataToInsert);
